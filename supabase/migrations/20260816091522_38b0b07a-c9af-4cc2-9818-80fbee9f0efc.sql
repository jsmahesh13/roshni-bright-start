-- Re-seed the demo dataset with dates rebased to today and consistent pronouns.
...
BEGIN
  FOR cls IN SELECT id, name FROM public.classes ORDER BY name LOOP
    FOR i IN 1..22 LOOP
      ...
      -- how long since the last noticing, and how far back the record stretches
      ...
      FOR j IN 1..n LOOP
        -- oldest first; the newest entry lands at last_gap days ago
        ...
        -- pronouns that match this child
...
-- Let the byline demonstrate itself: three recent 7B noticings written by Meena Rao.
